...
df<-expand.grid(x = seq(-3*pi,3*pi,0.1),y=seq(-3*pi,3*pi,0.1)) |> mutate(z= (sin(x)+sin(y)) - 0.5 * sqrt(x^2 + y^2))
ggplot(data = df) + geom_contour_filled(aes(x,y,z=z),breaks = c(-Inf,-6.8,-5.5,-4.5,-3.25,-2.4,-1.5,-0.40,0, 0.5,Inf))+scale_fill_hue()+coord_equal()+theme_void()+theme(legend.position="none")

ggsave("experimental/tootable-sample-4.png", width = 7, height = 7)
...
df<-expand.grid(x = seq(-3*pi,3*pi,0.1),y=seq(-3*pi,3*pi,0.1)) |> mutate(z= (sin(x/3)-sin(y/5)))
ggplot(data = df) + geom_contour_filled(aes(x,y,z=z),breaks = c(-Inf,-6.8,-5.5,-4.5,-3.25,-2.4,-1.5,-0.40,0, 0.5,Inf))+scale_fill_viridis_d()+coord_polar()+theme_void()+theme(legend.position="none")

ggsave("experimental/tootable-sample-5.png", width = 7, height = 7)
...
df <- df |>
  mutate(theta = 2 * pi * (x-min_x)/range_x,
         x_p = y * sin(theta),
         y_p = y * cos(theta))

p1 <- ggplot(data = df, aes(x = x_p, y = y_p, color = z)) + geom_point() + scale_color_viridis_c(option="H") + coord_equal() + theme(legend.position = "none")

plot_gg(p1, width = 7, height = 7, scale = 300, multicore = TRUE, windowsize = c(1000, 1000))
render_camera(fov = 70, zoom = 0.75, theta = -30, phi = 45)
render_snapshot(filename = "experimental/tootable-sample-5-3D.png", clear = TRUE)
...
df<-expand.grid(x = seq(-0*pi,3*pi,0.1),y=seq(-0*pi,3*pi,0.1)) |> mutate(z= (0.5 * y - sin(y) + sin(x)))
ggplot(data = df) + geom_contour_filled(aes(x,y,z=z),breaks = c(-Inf,-0.8,-0.0,1.5,2.25,2.4,3.5,4.40,Inf))+scale_fill_viridis_d(option="H")+coord_polar()+theme_void()+theme(legend.position="none")

ggsave("experimental/tootable-sample-6.png", width = 7, height = 7)
...
plot_gg(p1, width = 7, height = 7, scale = 300, multicore = TRUE, windowsize = c(1000, 1000))
render_camera(fov = 70, zoom = 0.75, theta = -30, phi = 45)
render_snapshot(filename = "experimental/tootable-sample-6-3D.png", clear = TRUE)
...
df<-expand.grid(x = seq(-0*pi,3*pi,0.1),y=seq(-0*pi,3.5*pi,0.1)) |> mutate(z= (-0.05 * y + sin(y/2) - sin(x/3)))
ggplot(data = df) + geom_contour_filled(aes(x,y,z=z),breaks = c(-Inf,-0.5,-0.2,0,0.25,0.75,1.2,1.7,2.40,Inf))+scale_fill_viridis_d(option="H")+coord_polar()+theme_void()+theme(legend.position="none")

ggsave("experimental/tootable-sample-7.png", width = 7, height = 7)
...
plot_gg(p1, width = 7, height = 7, scale = 300, multicore = TRUE, windowsize = c(1000, 1000))
render_camera(fov = 70, zoom = 0.75, theta = -30, phi = 45)
render_snapshot(filename = "experimental/tootable-sample-7-3D.png", clear = TRUE)
...
df<-expand.grid(x = seq(-0*pi,3*pi,0.1),y=seq(-0*pi,1.5*pi,0.1)) |> mutate(z= (0.5 * y - sin(y) + sin(x)))
ggplot(data = df) + geom_contour_filled(aes(x,y,z=z),breaks = c(-Inf,-0.8,-0.0,1.5,2.25,2.4,3.5,4.40,Inf))+scale_fill_viridis_d(option="H")+coord_polar()+theme_void()+theme(legend.position="none")

ggsave("experimental/tootable-sample-8.png", width = 7, height = 7)
...
The preceding maps incorporate only one element of randomness, the colors assigned to the filled contours. Here I modify the system (using a parametric equation similar to that used in Figure 5e in Puc and Skrekovski) to increase the range of emergent outcome. To do this, I add randomness to several features of the equation that generates the surface, to the selection of cut points, and the choice of color palette.

Note that when variables `a`, `b`, and `c` are 1, and variables `p`, `q`, and `r` are also 1, the equation is identical to the one used to create Figure 5e in Puc and Skrekovski.
```{r fig.show='hide'}
