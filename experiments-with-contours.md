...
ggsave("tootable-sample.png", height = 5, width = 5)
-->
...
library(dplyr)
library(ggplot2)
ggplot(data=expand.grid(x=seq(-3.5*pi,2.5*pi,0.1),y=seq(-3.5*pi,2.5*pi,0.1)) |> mutate(z=-sin(x)+sin(x)^3*sin(y)^3-sin(y)))+
geom_contour_filled(aes(x,y,z=z),breaks=c(-1.75,-0.25,0,0.25,1,Inf))+
geom_contour(aes(x=x+0.10,y=y-0.10,z=z),breaks = c(-1.25,-0.65,0.1),color="white")+
scale_fill_viridis_d(option="B")+coord_equal()+theme_void()+theme(legend.position="none")

ggsave("tootable-sample-3.png", height = 5, width = 5)
-->
...
While similar, the contour map is a lot less angular when we choose to see a different set of contours. The character of the map certainly seems to change:
...
Two data frames store the grid points in $D = {(x,y);-3.5\pi\le x \le 2.5\pi,-3.5\pi\le y \le 2.5\pi}$, as well as the values of $z_1$ and $z_2$:
